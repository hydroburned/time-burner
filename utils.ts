...
// HELPER: Recursively remove undefined values which break Firestore
...
export const sanitizeForFirestore = (state: any): any => {
    ...
    // 2. Deep clone via JSON to strip functions and symbols
    const jsonClean = JSON.parse(JSON.stringify(rawData));

    // 3. Recursive pass to remove explicit 'undefined' if JSON left any (rare but safe)
    return removeUndefined(jsonClean);
};
